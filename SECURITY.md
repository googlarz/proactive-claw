...
|--------|-------------|----------|-----------|
| `scripts/setup.sh` | Validates local Python dependencies, runs OAuth flow, creates Actions calendar, writes `config.json` | Google OAuth + Calendar API | User only, no sudo |
...
Third-party integrations are shipped in the separate `proactive-claw-integrations` add-on.
