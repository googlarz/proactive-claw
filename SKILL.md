...
primaryEnv: GOOGLE_OAUTH_CREDENTIALS_JSON
...
requires:
  ...
  config:
    - credentials.json (Google OAuth desktop flow via setup.sh OR Nextcloud app password)
    - config.json (generated by setup.sh — backend, timezone, autonomy level)
