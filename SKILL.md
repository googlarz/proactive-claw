...
name: proactive-claw
version: 1.2.17
description: "🦞 Proactive calendar AI: works with Google Calendar and Nextcloud. Auto-plans reminders, prep blocks and buffers. All features OFF by default."
...
install:
  - kind: uv
    label: "Google Calendar (default) — credentials.json required, run scripts/setup.sh"
    package: google-api-python-client
  - kind: uv
    label: "Nextcloud CalDAV (alternative) — app password required, run scripts/setup.sh"
    package: caldav

side_effects:
  - Requires credentials.json (Google OAuth desktop flow) OR Nextcloud app password before use. Run scripts/setup.sh to configure.
  - User-level daemon via launchd/systemd (non-root, every 15 min) installed via scripts/install_daemon.sh. Uninstall: see SKILL.md.
  ...
  - Creates one calendar "Proactive Claw — Actions" in Google/Nextcloud. All other calendars are read-only — never modified.
  - Outbound HTTPS: Google Calendar API only by default. Notion/Telegram/GitHub/LLM require explicit feature_* opt-in in config.json.
  - pip installs: google-api-python-client, google-auth-oauthlib, google-auth-httplib2 (Google) or caldav, icalendar (Nextcloud).
---

# 🦞 Proactive Claw v1.2.17
...
## ⚠️ Credential Requirements (declared in frontmatter)

| Credential | Required? | How to obtain |
|------------|-----------|---------------|
| `credentials.json` (Google OAuth) | **Required** for Google backend | Google Cloud Console → OAuth 2.0 desktop app, OR via clawhub OAuth integration |
| Nextcloud app password | **Required** for Nextcloud backend | `your-nextcloud.com/settings/personal/security` → App passwords |
| `config.json` | **Required** | Auto-generated by `scripts/setup.sh` (never copy manually) |
| `clawhub_token` | Optional | clawhub.ai/settings/integrations — only if using clawhub OAuth flow |
| Telegram bot token | Optional | Only if `feature_telegram_notifications: true` |
| LLM API key | Optional | Only if `llm_rater.base_url` points to a cloud endpoint |

**All optional credentials default to empty. Only the primary credential (credentials.json or Nextcloud password) is required to use the skill.**
